...
void parseirc(char *serveroutput)
{
	...
	if(!strcasecmp(command, "001")) {
		...
		/* store the real server name so that sstats shows it correctly */
	...
	if(!strcasecmp(command, "352")) {
		/* returned by the WHO query initiated in isjoined() */
		if((this = getchptr(trailing)) == NULL) {
			oerperl_call_eventhandlers(prefix, command, params, trailing);
			return;
		}
	...
	if(!strcasecmp(command, "JOIN")) {
		...
		if((msgcount = getondiskmsgcount(this, temp_nick)) <= 0) {
			oerperl_call_eventhandlers(prefix, command, params, trailing);
			return;
		}
		strncpy(temp_nick2, temp_nick, STRINGLEN);
		gettimeofday(&tv1, NULL);
		if(getrandommsg(this, temp_nick2, STRINGLEN, getrandom(msgcount), outstring2, HUGESTRINGLEN) < 0) {
			oerperl_call_eventhandlers(prefix, command, params, trailing);
			return;
		}
	...
	if(!strcasecmp(command, "NICK")) {
		...
		strncpy(userhost, prefix + tpos, USERHOSTLEN);
		if(isme(temp_nick)) {
			/* oer changed it's nick either because it was 
...
void parsecommand(char *target, char *nick, char *userhost, char *commandline)
{
	...
	case OER_WHICHCOMMAND_TOPIC_DEL:
		...
		deltopic(this, (tochan) ? this->name : nick, tochan, nick, userhost, restptr);
		return;
	...
	case OER_WHICHCOMMAND_INFO:
		...
		snprintf(outstring, WRITE_BUFFER_LENGTH, "%s, %s -- oer was written in March of 2000. This version of oer was compiled %s %s. The C source of this oer version consists of %d lines totalling %d bytes.", OER_VERSION, OER_COPYRIGHT1, __DATE__, __TIME__, SOURCE_LINES, SOURCE_CHARS);
		sendreply((tochan) ? channel : nick, tochan, 0, OER_TIMED_PRIORITY_NORMAL, outstring);
		return;
	...
	case OER_WHICHCOMMAND_QUOTE:
		...
		if((msgcount = getondiskmsgcount(this, restptr)) <= 0) {
			return;
		}
	...
	case OER_WHICHCOMMAND_SSTATS:
		...
		if(!checkauth(this, userhost, nick, tochan, 1)) {
			return;
                ...
                snprintf(outstring, WRITE_BUFFER_LENGTH, "%s RX/TX bytes: %d/%d, link connected for %s", (strlen(mystate->current_server->serverhost) > 0) ? mystate->current_server->serverhost_r : mystate->current_server->serverhost, mystate->current_server->rx, mystate->current_server->tx, uptimestring);
                sendreply((tochan) ? channel : nick, tochan, 0, OER_TIMED_PRIORITY_NORMAL, outstring);
                return;
...
void parsenotice(char *nick, char *userhost, char *target, char *message)
{
	struct channel * this;
#ifdef OER_DEBUG
        oer_debug(OER_DEBUG_INFO, "parsenotice(\"%s\", \"%s\", \"%s\", \"%s\")\n", nick, userhost, target, message);
#endif
	/* check whether the notice is sent to a channel or not */
	if((this = getchptr(target)) == NULL) {
		return;
	}
	if(this->i_am_op && checkforansi(this, nick, userhost, message)) {

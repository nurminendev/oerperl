...
void reconnect()
{
	...
	/* this is a workaround for the situation where admin changes oer's
	   nick to a taken one and then jumps/reconnects, oer should at least
	...
	case OER_RECONNECT_ERROR:
		if(mystate->quitting) {
#ifdef OER_DEBUG
			oer_debug(OER_DEBUG_INFO, "reconnect->oer exiting\n");
#endif
			close(mystate->sockfd);
			exit(EXIT_SUCCESS);
		}
...
int handleserverdata()
{
	...
	if(index(mystate->flags, (int)'l') != NULL) {
		...
		if(mystate->rawcounter >= OER_FORCE_FLUSH) {
			if(fflush(mystate->logfp) == EOF) {
#ifdef OER_DEBUG
				oer_debug(OER_DEBUG_ERROR, "handleserverdata->fflush() failed: %s", strerror(errno));
#endif
			}
			mystate->rawcounter = 0;
		}
	}
